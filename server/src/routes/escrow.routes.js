...
router.get('/transactions', async (req, res) => {
  try {
    ...
    query += ' GROUP BY et.id, o.order_number, bp.name, sp.name ORDER BY et.created_at DESC';
...
router.post('/transactions', async (req, res) => {
  try {
    ...
    const orderCheck = await pool.query('SELECT * FROM orders WHERE id = $1 AND buyer_id = $2', [order_id, buyer_id]);
    ...
    const result = await pool.query(
      `INSERT INTO escrow_transactions 
       ...
       RETURNING *`,
      [transactionNumber, order_id, buyer_id, seller_id, amount, escrow_fee || 0]
    );
...
router.put('/transactions/:id/status', async (req, res) => {
  try {
    ...
    const validStatuses = ['locked', 'pending_inspection', 'released', 'disputed', 'refunded'];
    ...
    const isParticipant = transaction.buyer_id === req.user.id || transaction.seller_id === req.user.id;
    const isAdmin = req.user.role === 'superUser' || req.user.role === 'admin';
