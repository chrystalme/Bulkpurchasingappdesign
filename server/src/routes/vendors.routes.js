...
router.get('/:id/dashboard', authenticateToken, canAccessVendorDashboard, async (req, res) => {
  try {
    const { id } = req.params;
    
    // Check if user has permission to view this vendor's dashboard
    if (req.user.role === 'vendor' && req.user.vendor_id !== parseInt(id)) {
      return res.status(403).json({ error: 'You can only view your own vendor dashboard' });
    }
...
router.get('/:id/orders', authenticateToken, canAccessVendorDashboard, async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/:id/customers', authenticateToken, canAccessVendorDashboard, async (req, res) => {
  try {
    const { id } = req.params;
