...
import { authenticateToken, canManageUsers, canDeleteUsers } from '../middleware/auth.js';
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (req.user.id !== parseInt(id) && req.user.role !== 'superUser' && req.user.role !== 'admin') {
...
router.post('/', canManageUsers, [
  ...
  body('role').isIn(['superUser', 'admin', 'vendor', 'member']).withMessage('Invalid role'),
], async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [email, hashedPassword, name, role, avatar, vendorId || null, trustScore]
    );
