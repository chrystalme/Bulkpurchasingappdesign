...
import { authenticateToken, canAccessVendorDashboard } from '../middleware/auth.js';
...
router.post('/', authenticateToken, canAccessVendorDashboard, [
  body('name').notEmpty().withMessage('Product name is required'),
  body('bulk_price').isFloat({ min: 0 }).withMessage('Valid bulk price is required'),
  ...
  try {
    ...
    const { name, image, bulk_price, retail_price, moq, vendor_id, category } = req.body;
    
    // If user is a vendor, they can only create products for their own vendor
    if (req.user.role === 'vendor' && req.user.vendor_id !== parseInt(vendor_id)) {
    ...
    const result = await pool.query(
      `INSERT INTO products (name, image, bulk_price, retail_price, moq, vendor_id, category)
       ...
       RETURNING *`,
      [name, image || 'default-product', bulk_price, retail_price, moq, vendor_id, category]
    );
...
router.put('/:id', authenticateToken, canAccessVendorDashboard, async (req, res) => {
  try {
    const { id } = req.params;
    const { name, image, bulk_price, retail_price, moq, category, is_active } = req.body;
    
    // Check if product exists and user has permission
    const productCheck = await pool.query('SELECT vendor_id FROM products WHERE id = $1', [id]);
    ...
    if (req.user.role === 'vendor' && req.user.vendor_id !== productCheck.rows[0].vendor_id) {
...
router.delete('/:id', authenticateToken, canAccessVendorDashboard, async (req, res) => {
  try {
    const { id } = req.params;
