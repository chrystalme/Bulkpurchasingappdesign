...
interface WelcomeProps {
  ...
  /** Public browse targets — guests can window-shop without an account. */
...
export function Welcome({ onGetStarted, onNavigateToLogin, onNavigateToSignup, onBrowseProducts, onBrowseGroups }: WelcomeProps) {
  const [activeCategory, setActiveCategory] = useState<string | null>(null);

  const handleSignIn = onNavigateToLogin || onGetStarted;
  const handleSignUp = onNavigateToSignup || onGetStarted;
  const handleBrowseProducts = onBrowseProducts || handleSignIn;
  const handleBrowseGroups = onBrowseGroups || handleSignIn;
