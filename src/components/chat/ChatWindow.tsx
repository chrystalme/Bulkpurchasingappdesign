...
import { ArrowLeft, Send, Users, Store, MoreVertical, Phone, Video, Info, ShieldAlert } from 'lucide-react';
...
export function ChatWindow({ conversation, onBack }: ChatWindowProps) {
  ...
  const canSendMessages = conversation.type === 'group' || 
      ...
      setTimeout(() => {
        const randomMember =
          conversation.participants[
            Math.floor(Math.random() * (conversation.participants.length - 1)) + 1
          ];
          ...
          {conversation.type === 'group' && (
            <div className="bg-white rounded-lg p-3 border border-gray-200 mb-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-semibold text-gray-900">Group Members</span>
              </div>
...
function getAutoResponse(userMessage: string, type: 'vendor' | 'group'): string {
